...
describe('Parser', () => {
  ...
  describe('Basic sanity tests', function() {
    ...
    describe('suffix', () => {
      test`
        [1 0x1 0.123 0.1E-12 1_u32 0xfefaU]
      `;
    });
    test``;
    ...
    (def a #asdads/date "123-123")
    
    {:name :auth
     :squad :infosec
     :environments {:prod #asdads/prototypes-for [:prod :global]
                    :staging #asdads/prototypes-for [:staging :global]
                    :test #asdads/prototypes-for [:test :global]}
     :workload #asdads/workload-for [:cache-generic-legacy :large]
     :legacy-naming? true
     :replicated? true
     :deprecated {:id 1}
     :overrides {:staging {:all {:replicated? false
                                 :workload #asdads/workload-for [:cache-generic-legacy :medium]}}
                 :test {:all {:replicated? false
                              :squad :eng-prod
                              :workload #asdads/workload-for [:cache-generic-legacy :medium]}}}}
